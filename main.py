...
# Removed all TTS voice/pitch/rate DB functions
...
# Removed keep_recording function
...
def status_handler(message):
    ...
    try:
        ...
        # total_tts_conversions = processing_stats_collection.count_documents({"type": "tts"}) # REMOVED
        ...
        # total_tts_conversions = 0 # REMOVED
...
# Removed all TTS voice-related functions and dictionaries
...
def handle_text_for_tts_or_fallback(message):
    ...
    # Removed all TTS specific text handling
    bot.send_message(
        message.chat.id,
        "I only transcribe voice messages, audio files, video notes, or video files. "
        "Please send a supported file type for transcription."
    )
...
def fallback_non_text_or_media(message):
    ...
    bot.send_message(
        message.chat.id,
        "I only transcribe voice messages, audio files, video notes, or video files. Please send a supported type."
    )
...
def set_bot_commands():
    ...
    commands = [
        ...
        # BotCommand("text_to_speech", "Set Text to voice language & voice "), # REMOVED
        # BotCommand("voice_pitch", "Adjust voice pitch"), # REMOVED
        # BotCommand("voice_rate", "Adjust voice speed"), # REMOVED
