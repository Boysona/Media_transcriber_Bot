...
def handle_file(message):
    ...
    try:
        ...
        # Store transcription using the ORIGINAL media message_id
        ...
        # Pass the original message.message_id to the callback data
        ...
        if len(transcription) > 4000:
            ...
            with open(fn, 'rb') as doc:
                # When sending as a document, pass the original message_id in the reply_markup
                bot.send_document(
                    message.chat.id,
                    doc,
                    reply_to_message_id=message.message_id, # This links the document to the original media visually
                    reply_markup=buttons, # Buttons will now have the correct original message_id
...
def generate_language_keyboard(callback_prefix, original_message_id=None):
    """Generates an inline keyboard for language selection, including the original_message_id."""
    markup = InlineKeyboardMarkup(row_width=3)
    buttons = []
    for lang in LANGUAGES:
        cb_data = f"{callback_prefix}|{lang['name']}"
        if original_message_id is not None:
            cb_data += f"|{original_message_id}" # Append original_message_id here
...
def select_language_command(message):
    ...
    update_user_activity(message.from_user.id)
    # When calling from /language, there's no original message ID for transcription context
...
def callback_set_language(call):
    ...
    _, lang = call.data.split("|", 1) # Only lang is expected from set_lang
...
def button_translate_handler(call):
    ...
    uid = str(call.from_user.id)
    _, original_message_id_str = call.data.split("|", 1) # Get the original_message_id
    original_message_id = int(original_message_id_str)

    if uid not in user_transcriptions or original_message_id not in user_transcriptions[uid]:
    ...
    if preferred_lang:
        ...
        do_translate_with_saved_lang(call.message, uid, preferred_lang, original_message_id)
    else:
        markup = generate_language_keyboard("translate_to", original_message_id) # Pass original_message_id
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
...
def button_summarize_handler(call):
    ...
    if preferred_lang:
        ...
        do_summarize_with_saved_lang(call.message, uid, preferred_lang, original_message_id)
    else:
        markup = generate_language_keyboard("summarize_in", original_message_id) # Pass original_message_id
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
...
def callback_translate_to(call):
    ...
    lang = parts[1]
    # This message_id is the original_message_id now
    ...
    if original_message_id:
        do_translate_with_saved_lang(call.message, uid, lang, original_message_id)
    else:
        # Fallback if original_message_id is not passed (should not happen with current flow)
...
def callback_summarize_in(call):
    ...
    if original_message_id:
        do_summarize_with_saved_lang(call.message, uid, lang, original_message_id)
    else:
        # Fallback if original_message_id is not passed
...
def do_translate_with_saved_lang(message, uid, lang, original_message_id):
    """Performs the translation using Gemini API."""
    original = user_transcriptions.get(uid, {}).get(original_message_id, "")
    if not original:
    ...
    if len(translated) > 4000:
        ...
        with open(fn, 'rb') as doc:
            # Reply to the original media message if possible, otherwise the current message
            reply_to_target_id = original_message_id if message.message_id != original_message_id else message.chat.id # If the current message is the media message, reply to current chat
            bot.send_document(message.chat.id, doc, caption=f"Translation to {lang}", reply_to_message_id=reply_to_target_id)
        os.remove(fn)
    else:
        # Reply to the original media message if possible, otherwise the current message
        reply_to_target_id = original_message_id if message.message_id != original_message_id else message.chat.id # If the current message is the media message, reply to current chat
        bot.send_message(message.chat.id, translated, reply_to_message_id=reply_to_target_id)

def do_summarize_with_saved_lang(message, uid, lang, original_message_id):
    ...
    if len(summary) > 4000:
        ...
        with open(fn, 'rb') as doc:
            # Reply to the original media message if possible, otherwise the current message
            reply_to_target_id = original_message_id if message.message_id != original_message_id else message.chat.id # If the current message is the media message, reply to current chat
            bot.send_document(message.chat.id, doc, caption=f"Summary in {lang}", reply_to_message_id=reply_to_target_id)
        os.remove(fn)
    else:
        # Reply to the original media message if possible, otherwise the current message
        reply_to_target_id = original_message_id if message.message_id != original_message_id else message.chat.id # If the current message is the media message, reply to current chat
        bot.send_message(message.chat.id, summary, reply_to_message_id=reply_to_target_id)
...
def handle_translate(message):
    ...
    if not message.reply_to_message:
        return bot.send_message(message.chat.id, "❌ Please reply to a transcription message or a media message to translate it.")

    # Check if the replied message is an original media message or a transcription message
    # For now, let's assume reply to transcription message (either text or document)
    # The original message_id is still required for lookup in user_transcriptions
    replied_message_id = message.reply_to_message.message_id
    
    # We need to find the original media message ID. This is a bit tricky if only replying to a document.
    # A more robust solution might involve storing the document_message_id to original_media_message_id mapping.
    # For simplicity, if replying to a document, we assume the original_message_id was included in its caption or similar.
    # A better approach would be to ensure the user_transcriptions key is always based on the original media ID.
    # Given the current structure, if they reply to the document, we can't directly get the original media message ID
    # unless it was embedded in the document's caption or data.
    # For now, this logic will primarily work if they reply to the direct text transcription.
    # If they reply to the document, `user_transcriptions[uid].get(replied_message_id)` will likely fail.
    # The new inline button callback data fixes this for inline buttons.

    # For `/translate` and `/summarize` commands used as replies, we need to ensure the replied message
    # has a transcription associated with it. The `user_transcriptions` maps original media message_ids.
    # If the user replies to the `.txt` file, `replied_message_id` will be the `.txt` file's ID,
    # which is not in `user_transcriptions`.

    # To make /translate and /summarize commands more robust with .txt files,
    # we would need to store a mapping from the .txt file's message_id back to the original media's message_id.
    # For now, the inline buttons are the primary way to trigger actions on .txt files.
    
    # Let's check if the replied message ID itself is a transcription, or if it's a media message
    # whose transcription we can look up.
    
    # If the replied message *is* the original media message
    original_media_message_id_for_command = replied_message_id
    if original_media_message_id_for_command not in user_transcriptions.get(uid, {}):
        # This means the user replied to something that isn't the original media or a direct transcription.
        # This could be the .txt file message.
        # The current `user_transcriptions` structure doesn't easily allow mapping document message IDs back.
        bot.send_message(message.chat.id, "❌ Transcription not found for the replied message. Please reply to the original media message or use the buttons under the transcription.")
        return


    preferred_lang = user_language_settings.get(uid)
    if preferred_lang:
        do_translate_with_saved_lang(message, uid, preferred_lang, original_media_message_id_for_command)
    else:
        markup = generate_language_keyboard("translate_to", original_media_message_id_for_command)
        bot.send_message(
            message.chat.id,
...
def handle_summarize(message):
    ...
    if not message.reply_to_message:
        return bot.send_message(message.chat.id, "❌ Please reply to a transcription message or a media message to summarize it.")
    ...
    preferred_lang = user_language_settings.get(uid)
    if preferred_lang:
        do_summarize_with_saved_lang(message, uid, preferred_lang, original_media_message_id_for_command)
    else:
        markup = generate_language_keyboard("summarize_in", original_media_message_id_for_command)
        bot.send_message(
            message.chat.id,
...
def fallback(message):
    ...
    update_user_activity(message.from_user.id)
    # Only respond if it's a direct text message, not if it's a reply to a bot message
    if message.reply_to_message and message.reply_to_message.from_user.id == bot.get_me().id:
        return # Avoid replying to our own messages or inline button clicks with fallback

    # Check if it's a text message that's not a command
    if message.text and not message.text.startswith('/'):
        bot.send_message(message.chat.id, "Please send only voice, audio, or video files for transcription. Use commands like /help for more info.")
    elif not message.text: # If it's another content type like photo, sticker, document
        bot.send_message(message.chat.id, "I can only process voice messages, audio files, or video files.")
