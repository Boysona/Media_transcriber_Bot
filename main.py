...
def download_youtube_video(call):
    ...
    try:
        ...
        bot.edit_message_text(f"Error downloading: {e}", chat_id=call.message.chat.id, message_id=call.message.message_id)
        if chat_id in bot.icon_messages:
            bot.delete_message(chat_id, bot.icon_messages[chat_id])
            bot.icon_messages.pop(chat_id, None)
    finally:
        ...
        try:
            ...
            if is_youtube_url(url):
                ...
                resolutions = {f'{f["height"]}p': f['format_id']
                               for f in formats if f.get('vcodec') != 'none' and f.get('height') <= 1080 and f.get('ext') == 'mp4'}
                ...
                markup = InlineKeyboardMarkup(row_width=3)
                for res, fid in sorted(resolutions.items(), key=lambda x: int(x[0][:-1]), reverse=True): # Descending order
            ...
            else: # Other social media platforms
                ...
                if path:
                    ...
                    bot.send_message(user_id, "An error occurred during download. Please try again or check the link.")
        except Exception as e:
            logging.error(f"Error handling URL {url}: {e}")
            bot.send_message(user_id, f"An error occurred while processing your link. Please try again.")
            if user_id in bot.icon_messages:
                bot.delete_message(user_id, bot.icon_messages[user_id])
                bot.icon_messages.pop(user_id, None)
        finally:
