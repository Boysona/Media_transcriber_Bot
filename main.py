...
# Bot token
TOKEN = "7770743573:AAHDlDTlactC7KU2L6nT6bzW9ueDuIp0p4Q" # Keep your token as is
...
def tiktok_link_handler(message):
    update_user_activity(message.from_user.id)
    bot.set_message_reaction(chat_id=message.chat.id, message_id=message.message_id, reaction="👀") # Add reaction
...
def callback_download_tiktok(call):
    ...
    try:
        ydl_opts = {
            ...
            'format': 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/mp4', # Ensure best mp4 format
            ...
            'noplaylist': True, # Avoid downloading playlists if URL is part of one
        ...
        if os.path.exists(path): # Ensure the file exists before sending
        ...
        bot.send_message(call.message.chat.id, "⚠️ Failed to download TikTok video. It might be private or an error occurred.")
    finally:
...
def callback_transcribe_tiktok(call):
    ...
    try:
        ydl_opts = {
            'outtmpl': os.path.join(DOWNLOAD_DIR, '%(id)s.%(ext)s'),
            'format': 'bestaudio/best', # Prioritize audio for transcription
            'postprocessors': [{
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info = ydl.extract_info(url, download=True)
            # yt-dlp might download as .webm or other audio formats
            # We need to find the actual downloaded file path
            ...
            if 'entries' in info:
                ...
                downloaded_file = ydl.prepare_filename(info) # Fallback

            if not downloaded_file or not os.path.exists(downloaded_file):
                raise FileNotFoundError(f"Could not find downloaded audio for {url}")

            path = downloaded_file # Use the actual downloaded file path
        ...
        bot.send_message(call.message.chat.id, "⚠️ Failed to transcribe TikTok. It might be private or an error occurred.")
    finally:
        if 'path' in locals() and os.path.exists(path):
            os.remove(path)
...
def handle_file(message):
    ...
    if message.voice:
        ...
        file_ext = ".mp3" # Assuming audio files are commonly mp3, can be refined
    elif message.video or message.video_note:
        file_ext = ".mp4" # Assuming video files are commonly mp4, can be refined

    local_path = os.path.join(DOWNLOAD_DIR, f"{uuid.uuid4()}{file_ext}")
...
def fallback(message):
    update_user_activity(message.from_user.id)
    # The subscription check was removed, so all users can interact.
...
if __name__ == "__main__":
    ...
    set_bot_info()
    bot.delete_webhook()
    # Ensure this URL is your actual deployed Render URL
