...
total_voice_clips = 0
total_videos = 0
total_tiktok_downloads = 0 # Keeping this for historical data if needed, but TikTok functions are removed
...
def set_bot_info():
    ...
    bot.set_my_short_description(
        "Effortlessly transcribe, summarize, and translate your media files. Quick, accurate, and free!"
    )

    # Full description (What can this bot do?)
    bot.set_my_description(
        """Transform your voice messages, audio files, and videos into clear, concise text in an instant. This bot offers free, accurate transcription, summarization, and translation across multiple languages.

Simply send your media, and let the bot do the rest! Enjoy seamless, high-quality conversions for all your content.
"""
    )
...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        bot.send_message(
            message.chat.id,
            f"""Hey {name}! 👋 Welcome to the **Media Transcriber Bot**!

Got audio or video? I'll turn it into clear, accurate text instantly.

Here's what I can do:
✨ Transcribe voice messages, audio files, and videos.
📝 Summarize lengthy content.
🗣️ Translate text into different languages.

**Quick Tips:**
• For best results, use clear audio with minimal background noise.
• Max file size: 20MB.
• Supported formats: Voice messages, video messages, and audio files.

Ready to start? Send me your media!
"""
        )
...
1.  **Data We Process:**
    * **Media Files:** Voice messages, audio files, and video files you send are temporarily processed for transcription. These files are **deleted immediately** after processing. We do not store your media content.
...
def do_translate_with_saved_lang(message, uid, lang, message_id):
    ...
    prompt = f"Translate this text in a natural and simpler way ({lang})—not word-for-word—but like how a normal person would write. Use commas to break up sentences, don’t add any explanations or extra info, not even an introduction, just translate the original text directly without including anything else\n\n{original}"
...
def do_summarize_with_saved_lang(message, uid, lang, message_id):
    ...
    prompt = f"Summary this text in a natural and simpler way ({lang})—not word-for-word—but like how a normal person would write. Use commas to break up sentences, don’t add any explanations or extra info, not even an introduction, just summary the original text directly without including anything else\n\n{original}"
