...
# CHANGE: Added 'stop_event' to store a threading.Event for each admin's uptime thread
...
def set_bot_info():
    commands = [
        telebot.types.BotCommand("start", "Restart the bot and view uptime (Admin only)"),
...
# CHANGE: Added stop_event parameter
def update_uptime_message(chat_id, message_id, stop_event):
    global bot_start_time
    while not stop_event.is_set(): # CHANGE: Check stop_event instead of True
        try:
            ...
            if "message is not modified" in str(e):
                ...
                break # Break on other API errors
...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        with admin_uptime_lock:
            # CHANGE: Check if an existing uptime thread is running for this admin
            if ADMIN_ID in admin_uptime_message and admin_uptime_message[ADMIN_ID].get('stop_event'):
                # Set the stop event for the old thread to terminate it
                admin_uptime_message[ADMIN_ID]['stop_event'].set()
                logging.info(f"Stopping previous uptime thread for admin {ADMIN_ID}")
            
            sent_message = bot.send_message(message.chat.id, "Admin Panel and Uptime (updating live)...", reply_markup=keyboard)

            # CHANGE: Create a new stop_event for the new thread
            ...
            admin_uptime_message[ADMIN_ID] = {
                ...
                'stop_event': new_stop_event # CHANGE: Store the new stop_event
            }
            uptime_thread = threading.Thread(target=update_uptime_message, args=(message.chat.id, sent_message.message_id, new_stop_event)) # CHANGE: Pass new_stop_event
            uptime_thread.daemon = True # Allows the main program to exit even if this thread is running
            uptime_thread.start()
            # admin_uptime_message[ADMIN_ID]['thread'] = uptime_thread # No longer strictly necessary to store the thread object itself if we manage stopping via event
...
2.  **How We Use Your Data:**
    ...
    * To improve bot performance and understand usage patterns through anonymous, aggregated statistics.
