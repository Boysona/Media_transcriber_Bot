...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        welcome_message = (
            ...
            """🔹 Choose the language for your media files using the buttons below:"""
        )
        
        # Create a more visually appealing keyboard
        markup = InlineKeyboardMarkup(row_width=2)
        buttons = [
            InlineKeyboardButton("English 🇬🇧", callback_data="set_media_lang|English 🇬🇧"),
            InlineKeyboardButton("Arabic 🇸🇦", callback_data="set_media_lang|Arabic 🇸🇦"),
            InlineKeyboardButton("Spanish 🇪🇸", callback_data="set_media_lang|Spanish 🇪🇸"),
            InlineKeyboardButton("French 🇫🇷", callback_data="set_media_lang|French 🇫🇷"),
...
@bot.message_handler(commands=['trane']) # NEW COMMAND
...
@bot.message_handler(commands=['sumy']) # NEW COMMAND
...
@bot.message_handler(commands=['lange'])
...
def button_summarize_handler(call):
    ...
    if preferred_lang:
        ...
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text="Please select the language you want the summary in (type /sumy for more options, or press auto):",
            reply_markup=markup
        )
    bot.answer_callback_query(call.id)
...
def callback_summarize_in(call):
    ...
    if lang_display_name == "Auto ⚙️":
        ...
        if transcription_lang:
            ...
            bot.answer_callback_query(call.id, "Cannot determine original transcription language. Please select a specific language using /sumy.")
            # If "Auto" fails and no media_language is set, inform the user to use /sumy
            bot.edit_message_text(
                chat_id=call.message.chat.id,
                message_id=call.message.message_id,
                text="Cannot determine original transcription language. Please use /sumy to select a language, then try again."
            )
            return
    else:
...
def handle_translate(message):
    ...
    if transcription_message_id:
        ...
        bot.send_message(
            message.chat.id,
            "Please select the language you want to translate into, then reply to a transcription message with /trane to use it:",
            reply_markup=markup
        )
...
def handle_summarize(message):
    ...
    if transcription_message_id:
        ...
        bot.send_message(
            message.chat.id,
            "Please select 'Auto' for summary, or use /sumy to set your preferred summary language for future use:",
            reply_markup=markup
        )
    ...
    if user_tts_mode.get(uid):
        ...
        if saved_voice != "en-US-AriaNeural":
            ...
            bot.send_message(
                ...
                "To convert text to speech, use the /voice command first."
            )
...
def fallback_non_text_or_media(message):
    ...
    if message.document and not (message.document.mime_type.startswith("audio/") or message.document.mime_type.startswith("video/")):
        bot.send_message(
            ...
            "or use `/voice` for text to speech. The document type you sent is not supported for transcription."
        )
        return
    ...
    bot.send_message(
        ...
        "or use `/voice` for text to speech. Please send a supported type."
    )
