...
import asyncio
import subprocess  # Changed: Import subprocess
...
# Ensure this runs once when the bot starts
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        with admin_uptime_lock:
            if admin_uptime_message.get(ADMIN_ID) and admin_uptime_message[ADMIN_ID].get('thread') and admin_uptime_message[ADMIN_ID]['thread'].is_alive():
                pass # Already running, or relies on it breaking its loop
...
def handle_file_wrapper(message):
    ...
    """
    # Send a quick acknowledgement or "processing" message
    bot.send_chat_action(message.chat.id, 'typing')
    bot.send_message(message.chat.id, "Received your file, processing now... This may take a moment.")

    # Start the actual processing in a new thread
    thread = threading.Thread(target=process_media_file, args=(message,))
    thread.start()
...
def process_media_file(message):
    ...
    if file_obj.file_size > FILE_SIZE_LIMIT:
        bot.send_message(chat_id, "The file size you uploaded is too large (max allowed is 20MB).")
        return

    info = bot.get_file(file_obj.file_id)
    file_extension = ".ogg" if message.voice or message.video_note else os.path.splitext(info.file_path)[1]
    
    # Use a temporary file path for initial download
    temp_download_path = os.path.join(DOWNLOAD_DIR, f"{uuid.uuid4()}{file_extension}")
    wav_output_path = os.path.join(DOWNLOAD_DIR, f"{uuid.uuid4()}.wav") # Temporary WAV for transcription
    ...
    try:
        ...
        # Step 2: Convert to WAV using subprocess (in-memory is complex with faster-whisper)
        # Using subprocess for FFmpeg conversion
        ...
        try:
            ...
            bot.send_message(chat_id, "⚠️ Your file cannot be converted to the correct voice recognition format. FFmpeg error.")
            return # Exit function, cleanup in finally block
        except Exception as e:
            logging.error(f"FFmpeg conversion failed: {e}")
            bot.send_message(chat_id, "⚠️ Your file cannot be converted to the correct voice recognition format.")
            return # Exit function, cleanup in finally block
        ...
        bot.send_message(chat_id, "⚠️ An unexpected error occurred during transcription. Please try again.")
    finally:
