...
# ========================================
# MODERN UI COMPONENTS (REDESIGNED)
# ========================================
def generate_main_menu():
    ...
    buttons = [
        ...
        "⚙️ Language", "📊 Status", "🔄 Reset"
    ]
    markup.add(*buttons)
    return markup

def generate_language_keyboard(callback_prefix: str, message_id: int | None = None):
    """Create modern inline keyboard for language selection with flags"""
...
def make_tts_language_keyboard():
    """Modern TTS language keyboard with flags"""
    markup = InlineKeyboardMarkup(row_width=3)
    buttons = []
    ...
    markup.add(InlineKeyboardButton("⬅️ Back", callback_data="tts_back_to_settings"))    
    return markup
...
def make_pitch_keyboard(user_id: str):
    """Pitch adjustment keyboard with visual indicators"""
    current_pitch = get_tts_user_pitch_db(user_id)
    pitch_level = min(max((current_pitch + 100) // 40, 0), 5)
    
    # Create visual indicator
    indicator = "◼" * pitch_level + "◻" * (5 - pitch_level)
    
    markup = InlineKeyboardMarkup(row_width=3)
    markup.add(
        InlineKeyboardButton(f"High {indicator}", callback_data="pitch_set|+50"),
        InlineKeyboardButton("Normal", callback_data="pitch_set|0"),
        InlineKeyboardButton("Custom", callback_data="pitch_custom")
    )
    markup.add(InlineKeyboardButton("⬅️ Back", callback_data="tts_back_to_settings"))
    return markup

def make_rate_keyboard(user_id: str):
    """Speech rate adjustment keyboard with visual indicators"""
    current_rate = get_tts_user_rate_db(user_id)
    rate_level = min(max((current_rate + 100) // 40, 0), 5)
    
    # Create visual indicator
    indicator = "◼" * rate_level + "◻" * (5 - rate_level)
    
    markup = InlineKeyboardMarkup(row_width=3)
    markup.add(
        InlineKeyboardButton(f"Fast {indicator}", callback_data="rate_set|+50"),
        InlineKeyboardButton("Normal", callback_data="rate_set|0"),
        InlineKeyboardButton("Custom", callback_data="rate_custom")
    )
    markup.add(InlineKeyboardButton("⬅️ Back", callback_data="tts_back_to_settings"))
    return markup

def generate_divider():
    """Create a visual divider for messages"""
    return "━━━━━━━━━━━━━━"
...
def send_subscription_message(chat_id: int):
    if bot.get_chat(chat_id).type == 'private':
        ...
        bot.send_message(
            chat_id,
            f"""🔒 **Access Restricted**\n{generate_divider()}\nTo use this bot, please join our official channel:\n\n{REQUIRED_CHANNEL}\n\nAfter joining, click 'Check Subscription' below.""",
            reply_markup=markup,
            parse_mode="Markdown"
        )
...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        welcome_msg = (
            f"""🌟 **Welcome to MediaToText Pro!**\n{generate_divider()}\n"""
            f"""I help you convert voice messages, audio clips, and videos into text with powerful features:\n\n"""
            f"""• 🎤 High-quality transcription\n"""
            f"""• 🔊 Text-to-speech conversion\n"""
            f"""• 🌐 Multi-language translation\n"""
            f"""• 📝 AI-powered summarization\n\n"""
            f"""All services are completely free!"""
        )
        bot.send_message(
            message.chat.id,
            welcome_msg,
...
@bot.message_handler(func=lambda m: m.text in ["ℹ️ Help Center", "ℹ️ Help"])
def help_handler(message):
    ...
    help_text = (
        ...
        "   - Set language first with \"⚙️ Language\"\n\n"
        "2. **🌐 Translation**\n"
        "   - Set target language with \"🌐 Translation\"\n"
        "   - Reply to transcription with /translate\n\n"
        "3. **📝 Summarization**\n"
        "   - Set target language with \"📝 Summarization\"\n"
        "   - Reply to transcription with /summarize\n\n"
        "4. **🔊 Text-to-Speech**\n"
        "   - Configure with \"🔊 Voice Settings\"\n"
        "   - Send any text to convert\n\n"
        "5. **⚙️ Voice Customization**\n"
        "   - Adjust pitch and speed in Voice Settings\n\n"
        f"{generate_divider()}\n"
...
@bot.message_handler(func=lambda m: m.text == "⚙️ Language")
def select_media_language_command(message):
    ...
    bot.send_message(
        message.chat.id,
        f"🗣️ **Set Media Language**\n{generate_divider()}\nSelect the language for your media files:",
        parse_mode="Markdown",
        reply_markup=generate_language_keyboard("set_media_lang")
    )

@bot.message_handler(func=lambda m: m.text == "🔊 Voice Settings")
def cmd_text_to_speech(message):
    ...
    # Create modern inline keyboard with new buttons
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("🗣️ Voice Pitch", callback_data="tts_pitch_settings"),
        InlineKeyboardButton("⏩ Voice Rate", callback_data="tts_rate_settings"),
        InlineKeyboardButton("🌐 Change Voice", callback_data="tts_back_to_languages")
    )
...
@bot.message_handler(func=lambda m: m.text == "🌐 Translation")
...
def select_summary_language_command(message):
    ...
    bot.send_message(
        message.chat.id,
        f"📄 **Set Summary Language**\n{generate_divider()}\nSelect your preferred summary language:",
        parse_mode="Markdown",
        reply_markup=generate_language_keyboard("set_summary_lang")
    )

@bot.message_handler(func=lambda m: m.text in ["👤 My Profile", "👤 User Info"])
def cmd_userinfo(message):
    uid_str = str(message.from_user.id)
    update_user_activity_db(message.from_user.id)

    if message.chat.type == 'private' and uid_str != str(ADMIN_ID) and not check_subscription(message.chat.id):
        send_subscription_message(message.chat.id)
        return
    ...
    # Get language flags
    media_flag = LANGUAGE_FLAGS.get(get_lang_code(media_lang) or "", "")
    trans_flag = LANGUAGE_FLAGS.get(get_lang_code(trans_lang) or "", "")
    summary_flag = LANGUAGE_FLAGS.get(get_lang_code(summary_lang) or "", "")
    ...
    user_info = (
        ...
        f"⚙️ **Language Settings**\n{generate_divider()}\n"
        f"{media_flag} Media: `{media_lang}`\n"
        f"{trans_flag} Translation: `{trans_lang}`\n"
        f"{summary_flag} Summary: `{summary_lang}`\n\n"
        f"🔊 **Voice Settings**\n{generate_divider()}\n"
        f"🗣️ Voice: `{tts_voice}`\n"
        f"🎚️ Pitch: `{tts_pitch}`\n"
        f"⏩ Rate: `{tts_rate}`"
...
@bot.message_handler(func=lambda m: m.text in ["📊 Status", "📊 Statistics"])
...
def handle_file(message):
    ...
    processing_reply = bot.reply_to(message, "🔄 Processing your media...")
    ...
    try:
        ...
        if not media_lang_name:
            bot.edit_message_text(
                ...
                text="⚠️ Set media language first using \"⚙️ Language\""
            )
            return
...
def callback_set_translation_language(call):
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text=f"✅ Translation language set to: {flag} **{lang_display_name}**",
        parse_mode="Markdown"
    )
    bot.answer_callback_query(call.id, text=f"Set to {lang_display_name}")
...
def callback_set_summary_language(call):
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text=f"✅ Summary language set to: {flag} **{lang_display_name}**",
        parse_mode="Markdown"
    )
    bot.answer_callback_query(call.id, text=f"Set to {lang_display_name}")
...
def callback_translate_to(call):
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text=f"{flag} Translating to **{lang_display_name}**...",
        parse_mode="Markdown"
    )
...
def callback_summarize_in(call):
    ...
    if lang_display_name == "Auto ⚙️":
        ...
        if transcription_lang:
            ...
            lang_code = get_lang_code(transcription_lang)
            flag = LANGUAGE_FLAGS.get(lang_code, "") if lang_code else ""
            
            bot.edit_message_text(
                chat_id=call.message.chat.id,
                message_id=call.message.message_id,
                text=f"{flag} Summarizing in **{transcription_lang}**...",
                parse_mode="Markdown"
            )
        else:
        ...
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text=f"{flag} Summarizing in **{lang_display_name}**...",
            parse_mode="Markdown"
        )
...
def do_translate_with_saved_lang(message, uid, lang_display_name, message_id):
    ...
    if len(translated) > 4000:
        ...
        bot.send_document(
            message.chat.id, 
            translation_file_buffer, 
            caption=f"{flag} {lang_display_name} Translation", 
        ...
        bot.send_message(
            message.chat.id, 
            f"{flag} **{lang_display_name} Translation**\n{generate_divider()}\n{translated}",
            parse_mode="Markdown",
            reply_to_message_id=message_id
        )
...
def do_summarize_with_saved_lang(message, uid, lang_display_name, message_id):
    ...
    if len(summary) > 4000:
        ...
        bot.send_document(
            message.chat.id, 
            summary_file_buffer, 
            caption=f"{flag} {lang_display_name} Summary", 
            reply_to_message_id=message_id
        )
        ...
        bot.send_message(
            message.chat.id, 
            f"{flag} **{lang_display_name} Summary**\n{generate_divider()}\n{summary}",
            parse_mode="Markdown",
            reply_to_message_id=message_id
        )
...
def on_tts_voice_change(call):
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text=f"🔊 **Voice Settings**\n{generate_divider()}\n"
             f"🗣️ Voice: *{voice}*\n"
             f"🎚️ Pitch: *{current_pitch}*\n"
             f"⏩ Rate: *{current_rate}*\n\n"
             f"Send text to convert to speech.",
        parse_mode="Markdown"
    )
...
@bot.callback_query_handler(func=lambda c: c.data == "tts_pitch_settings")
def on_tts_pitch_settings(call):
    uid = str(call.from_user.id)
    update_user_activity_db(call.from_user.id)
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text=f"🎚️ **Voice Pitch Settings**\n{generate_divider()}\nSelect preset or enter value (-100 to +100):",
        reply_markup=make_pitch_keyboard(uid)
    )
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data == "tts_rate_settings")
def on_tts_rate_settings(call):
    uid = str(call.from_user.id)
    update_user_activity_db(call.from_user.id)
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text=f"⏩ **Speech Rate Settings**\n{generate_divider()}\nSelect preset or enter value (-100 to +100):",
        reply_markup=make_rate_keyboard(uid)
    )
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data == "tts_back_to_settings")
def on_tts_back_to_settings(call):
    uid = str(call.from_user.id)
    update_user_activity_db(call.from_user.id)
    ...
    # Create the main Voice Settings menu
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("🗣️ Voice Pitch", callback_data="tts_pitch_settings"),
        InlineKeyboardButton("⏩ Voice Rate", callback_data="tts_rate_settings"),
...
@bot.callback_query_handler(func=lambda c: c.data == "pitch_custom")
def on_pitch_custom(call):
    uid = str(call.from_user.id)
    bot.send_message(
        call.message.chat.id,
        "🎚️ Enter pitch value (-100 to +100):"
    )
    user_pitch_input_mode[uid] = "awaiting_pitch_input"
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data == "rate_custom")
def on_rate_custom(call):
    uid = str(call.from_user.id)
    bot.send_message(
        call.message.chat.id,
        "⏩ Enter rate value (-100 to +100):"
    )
...
def handle_text_for_tts_or_fallback(message):
    ...
    if user_tts_mode.get(uid):
        ...
        if saved_voice != "en-US-AriaNeural":
            ...
            bot.send_message(
                message.chat.id,
                "📝 Send audio/video for transcription or configure voice settings",
                reply_markup=generate_main_menu()
            )
