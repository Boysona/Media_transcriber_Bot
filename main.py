...
# Admin uptime message storage - Keeping this, but not triggered by /start for admin
...
# Function to update uptime message (not directly called by /start anymore for admin)
def update_uptime_message(chat_id, message_id):
    """
    Live-update the admin uptime message every second, showing days, hours, minutes and seconds.
    This function will still exist but will be explicitly triggered by an admin command if desired,
    not automatically on /start.
    """
    while True:
        try:
...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        # Admin specific start message and keyboard
        keyboard = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
        keyboard.add("Send Broadcast", "Total Users", "/status")
        bot.send_message(message.chat.id, "👋 Welcome, Admin! You can manage the bot using the keyboard below.", reply_markup=keyboard)
        # Removed the uptime message and thread initiation for admin on /start
        # If you want to show uptime to admin, consider a separate command like /admin_uptime
    else:
        # Tani waa qaybta lagu soo dhiibayo welcome message-ka user-ka caadiga ah.
        display_name = message.from_user.first_name or (f"@{message.from_user.username}" if message.from_user.username else "user")
        
        # Enhanced Welcome Message
        welcome_text = f"""
👋🏻 Welcome dear {display_name}!
I’m your advanced media transcription bot.

🎙️ **What I can transcribe:**
• Voice Messages
• Video Messages
• Audio Files (MP3, WAV, M4A, etc.)

📝 **How it works:**
Simply send your media file to me. I will automatically detect the language and transcribe it for free!
✨ Powered by the **Faster-Whisper `base` model**, I offer efficient and accurate transcriptions.

⚠️ **Important Notes:**
• The maximum file size allowed is **{FILE_SIZE_LIMIT / (1024 * 1024):.0f}MB**.
• For the best transcription results, please ensure your recordings are:
    * **Clear**: Speak distinctly.
    * **Calm**: Minimize background noise.
    * **Error-free**: Avoid choppy audio or corrupted files.

If you need any help, feel free to contact ☎️ @botmakerrr.
"""
        bot.send_message(
            message.chat.id,
            welcome_text,
    ...
    try:
        ...
        if file_obj.file_size > FILE_SIZE_LIMIT:
            bot.edit_message_text(
                ...
                text=f"⚠️ The file size you uploaded is too large (max allowed is {FILE_SIZE_LIMIT / (1024 * 1024):.0f}MB).",
