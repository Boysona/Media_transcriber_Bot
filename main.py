...
import asyncio
import speech_recognition as sr # Keep this for AudioFile handling, but recognition will change
...
# Choose a model size: "tiny", "base", "small", "medium", "large", "large-v1", "large-v2", "large-v3"
# You can also specify a compute type for performance, e.g., "int8" for CPU, "float16" for GPU
# For this example, we'll use "tiny" for efficiency.
try:
    WHISPER_MODEL = WhisperModel("small", device="cpu", compute_type="int8") # Use "cpu" and "int8" for broader compatibility
...
def process_media_file(message, stop_typing):
    ...
    if file_obj.file_id in transcription_cache:
        ...
        wav_audio_data_path = None # Path to the temporary WAV file for Whisper
        ...
        try:
            ...
            # Convert to WAV using subprocess for faster-whisper input
            wav_audio_data_path = os.path.join(DOWNLOAD_DIR, f"{uuid.uuid4()}.wav")
            try:
                command = [
                ...
                if not os.path.exists(wav_audio_data_path) or os.path.getsize(wav_audio_data_path) == 0:
            ...
            # 2) **Transcribe with faster-whisper**: Use the loaded model
            if WHISPER_MODEL is None:
                bot.send_message(message.chat.id, "❌ Transcription service is unavailable (Whisper model not loaded). Please try again later.")
                return

            try:
                # faster-whisper expects a file path, so we use the temporary WAV file
                segments, info = WHISPER_MODEL.transcribe(
                    wav_audio_data_path,
                    beam_size=5,
                    language=media_lang_code.split('-')[0], # Whisper expects language code like 'en', 'es'
                ...
                logging.info(f"Whisper transcription done. Detected language: {info.language}")
                ...
                bot.send_message(message.chat.id,
                                 "😓 Sorry, I couldn’t understand the audio using faster-whisper. Please try with a clearer recording or a different file.")
                return
            ...
            # Clean up the temporary WAV file created for Whisper
            if wav_audio_data_path and os.path.exists(wav_audio_data_path):
                os.remove(wav_audio_data_path)
                logging.info(f"Cleaned up {wav_audio_data_path}")
...
# The old transcribe_audio_from_bytes_with_retry function is now unused.
# You can remove it or keep it as a reference if you want to switch back.
# For faster-whisper, we directly use WHISPER_MODEL.transcribe with the file path.
# Leaving the old one for completeness, but it won't be called.
        ...
        while start_ms < total_length_ms:
            ...
            with sr.AudioFile(chunk_io) as source:
                ...
                while attempt < retries and not success:
                    try:
                        # This line will be replaced by faster-whisper in the actual processing flow
