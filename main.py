    ...
    try:
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                info = ydl.extract_info(url, download=False)
                description = info.get('description', 'No description available.')
                # Download the video after getting info
                ydl.download([url])
        except Exception as e:
            print(f"Error during info extraction or initial download for {url}: {e}")
        ...
        bot.send_message(msg.chat.id, f"Error downloading social media video: {e}")
