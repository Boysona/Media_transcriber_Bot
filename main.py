...
import asyncio
# import speech_recognition as sr # Removed: Replaced by faster-whisper
...
# User-specific media language settings for speech recognition - REMOVED, as faster-whisper auto-detects
...
# Removed: save_user_media_language_settings as it's no longer needed
...
def set_bot_info():
    commands = [
        ...
        # telebot.types.BotCommand("media_language", "📝Set language for media transcription"), # Removed
...
def handle_file(message):
    ...
    # Removed: No need to check for user_media_language_settings as faster-whisper auto-detects
    ...
    try:
        ...
        try:
            # Ensure the output format is suitable for faster-whisper (e.g., 16kHz WAV)
        ...
        # Removed: Language code check is no longer needed for transcription with faster-whisper
        ...
        # Call transcribe_audio_chunks without language parameter
        transcription = transcribe_audio_chunks(wav_path) or ""
...
# Note: The LANGUAGES list and get_lang_code function are still used for
# Gemini API calls (translation/summarization) which often benefit from
# explicit language codes, even if Whisper auto-detects for transcription.
LANGUAGES = [
...
# Removed: @bot.message_handler(commands=['media_language']) and related callback handlers
# as faster-whisper handles automatic language detection for transcription.
