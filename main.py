...
TOKEN = "7790991731:AAFgEjc6fO-iTSSpt3lEJBH86gQY5nIgAw"  # <-- your bot token
...
# --- Helper to generate main Reply Keyboard ---
def generate_main_keyboard():
    keyboard = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    # Autonyms for clarity based on previous request for autonyms for language display.
    # Here, for the main menu, keeping English for command readability on buttons.
    keyboard.add(KeyboardButton("🗣️ Media Language"),
                  KeyboardButton("📝 Translate Language"))
    keyboard.add(KeyboardButton("📚 Summary Language"),
                  KeyboardButton("🔊 Text-to-Speech"))
    keyboard.add(KeyboardButton("ℹ️ User Info"), # New Button
                  KeyboardButton("❓ Help"))
    keyboard.add(KeyboardButton("🔒 Privacy"),
...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        # Admin keyboard includes broadcast and total users for quick access
        admin_keyboard = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
        admin_keyboard.add("Send Broadcast", "Total Users", "/status")
        sent_message = bot.send_message(
            message.chat.id,
            "✨ Welcome Admin! Your dashboard and bot uptime are updating live below.",
        ...
        # Send the new welcome message with the main reply keyboard
        welcome_message = (
            f"👋 Welcome, {message.from_user.first_name}!\n\n"
            "I'm your friendly **Media Transcriber Bot**! 🤖\n"
            "Send me any voice message, audio file, or video (up to 20MB) "
            "and I'll transcribe it for you. You can then translate or summarize the text.\n\n"
            "**💡 Quick Tip:** First, set your media's language using the '🗣️ Media Language' button below for best transcription accuracy! Or, simply send your media and I'll try to detect the language automatically."
        )
        bot.send_message(
            message.chat.id,
            welcome_message,
            reply_markup=generate_main_keyboard(), # Use the new main keyboard
            parse_mode="Markdown"
        )


@bot.message_handler(commands=['help'], regexp='^/help$')
...
4.  **Text-to-Speech**
    • Use /Langtts (or the '🔊 Text-to-Speech' button) to pick a language and voice first.
...
5.  **Language Settings**
    • /bottomtrnlang (or '📝 Translate Language') — set your preferred language for translations.
    • /bottomsumlang (or '📚 Summary Language') — set your preferred language for summaries.
    • /Langstt (or '🗣️ Media Language') — set the language of any audio/video you send for transcription.

6.  **User Info**
    • Use /userinfo (or 'ℹ️ User Info') — view your saved preferences and usage statistics.
...
@bot.message_handler(commands=['privacy'], regexp='^/privacy$')
...
@bot.message_handler(commands=['status'], regexp='^/status$')
...
@bot.message_handler(commands=['userinfo'], regexp='^/userinfo$')
@bot.message_handler(func=lambda m: m.text == "ℹ️ User Info")
def handle_user_info(message):
    ...
    # Ensure TTS modes are OFF
    user_tts_mode[uid] = None
    user_pitch_input_mode[uid] = None
    user_rate_input_mode[uid] = None

    user_doc = get_user_data_db(uid)
    if not user_doc:
        bot.send_message(message.chat.id, "❌ Could not retrieve your user information.")
        return

    last_active = datetime.fromisoformat(user_doc.get("last_active", datetime.min.isoformat()))
    transcription_count = user_doc.get("transcription_count", 0)

    # Fetch language settings and TTS settings
    media_lang = get_user_media_language_setting_db(uid) or "Not set (auto-detect by default)"
    translation_lang = get_user_translation_language_db(uid) or "Not set"
    summary_lang = get_user_summary_language_db(uid) or "Not set"
    tts_voice = get_tts_user_voice_db(uid)
    tts_pitch = get_tts_user_pitch_db(uid)
    tts_rate = get_tts_user_rate_db(uid)

    # Get user-specific processing stats
    try:
        user_total_media_processed = processing_stats_collection.count_documents({"user_id": uid, "type": {"$in": ["voice", "audio", "video"]}})
        user_total_tts_conversions = processing_stats_collection.count_documents({"user_id": uid, "type": "tts"})
        
        pipeline = [
            {"$match": {"user_id": uid}},
            {"$group": {"_id": None, "total_user_time": {"$sum": "$processing_time"}}}
        ]
        agg_result = list(processing_stats_collection.aggregate(pipeline))
        user_total_proc_seconds = agg_result[0]["total_user_time"] if agg_result else 0
    except Exception as e:
        logging.error(f"Error fetching user processing stats for {uid}: {e}")
        user_total_media_processed = user_total_tts_conversions = 0
        user_total_proc_seconds = 0
    
    user_proc_hours = int(user_total_proc_seconds) // 3600
    user_proc_minutes = (int(user_total_proc_seconds) % 3600) // 60
    user_proc_seconds = int(user_total_proc_seconds) % 60
    ...
    info_text = (
        ...
        f"🗣️ Transcriptions Processed: {transcription_count}\n\n"
        "--- **Your Preferences** ---\n"
        f"⚙️ Media Language: **{media_lang}** (`/Langstt`)\n"
        f"📝 Translate Language: **{translation_lang}** (`/bottomtrnlang`)\n"
        f"📚 Summary Language: **{summary_lang}** (`/bottomsumlang`)\n\n"
        "--- **Text-to-Speech Settings** ---\n"
        f"🎤 Current Voice: `{tts_voice}`\n"
        f"⬆️ Pitch: `{tts_pitch}` (`/voice_pitch`)\n"
        f"⚡ Rate: `{tts_rate}` (`/voice_rate`)\n"
        f"Total TTS Conversions: {user_total_tts_conversions}\n\n"
        "--- **Your Usage Stats** ---\n"
        f"Total Media You Processed: {user_total_media_processed}\n"
        f"Your Total Processing Time: {user_proc_hours}h {user_proc_minutes}m {user_proc_seconds}s\n\n"
        "—\n"
        "Manage your settings with the commands above or through the main menu buttons."
    )
    ...
    try:
        ...
        if not media_lang_name: # Handle case where media_language might not be set
             bot.edit_message_text(
                ...
                text="⚠️ Transcription failed. Please set your media language using /Langstt before sending files."
            )
             return # Exit early
        ...
        if "400 Client Error: Bad Request for url" in str(req_e) and "language_code" in str(req_e):
             error_message = (
                ...
                "Please choose a different language using /Langstt or try again with a different file."
             )
        # Delete the "Processing..." message
        try:
            bot.delete_message(message.chat.id, processing_message_id)
...
def get_lang_code(lang_name: str) -> str | None:
    ...
    for key, code in LANGUAGES.items():
        ...
        if lang_name.lower() in key.lower() or lang_name == key: # Added exact key match
            return code
    return None
...
def generate_language_keyboard(callback_prefix: str, message_id: int | None = None):
    ...
    - If callback_prefix is "summarize_in", only "Auto ⚙️" is shown initially.
      Other languages are assumed to be presented via /bottomsumlang.
    ...
    if callback_prefix == "summarize_in":
        # Only show "Auto" button for direct summarization action if /bottomsumlang hasn't been used
    ...
    # Arrange buttons to try for a 3-column appearance
...
# --- UPDATED: TTS VOICES BY LANGUAGE (Removed unwanted languages/voices, added autonyms) ---
TTS_VOICES_BY_LANGUAGE = {
    ...
    "हिन्दी 🇮🇳": [ # ONLY Hindi voices (removed Tamil, Telugu, Kannada, Malayalam, Gujarati, Marathi)
        "hi-IN-SwaraNeural", "hi-IN-MadhurNeural"
    ],
    "Français 🇫🇷": [
        "fr-FR-DeniseNeural", "fr-FR-HenriNeural", "fr-CA-SylvieNeural", "fr-CA-JeanNeural",
        "fr-CH-ArianeNeural", "fr-CH-FabriceNeural", "fr-BE-GerardNeural"
    ],
    "Deutsch 🇩🇪": [
...
ORDERED_TTS_LANGUAGES = [
    ...
    "简体中文 🇨🇳", "日本語 🇯🇵", "Português 🇧🇷", "Русский 🇷🇺", "Türkçe 🇹🇷",
...
def make_pitch_keyboard():
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("Fastest (+100%)", callback_data="pitch_set|+100"), # Changed to use +100 for fastest
        InlineKeyboardButton("Faster (+50%)", callback_data="pitch_set|+50"),
        InlineKeyboardButton("Normal (0%)", callback_data="pitch_set|0"),
        InlineKeyboardButton("Slower (-50%)", callback_data="pitch_set|-50"),
        InlineKeyboardButton("Slowest (-100%)", callback_data="pitch_set|-100") # Changed to use -100 for slowest
    )
    return markup
...
def make_rate_keyboard():
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("Fastest (+100%)", callback_data="rate_set|+100"), # Max rate is +100
        InlineKeyboardButton("Faster (+50%)", callback_data="rate_set|+50"),
        InlineKeyboardButton("Normal (0%)", callback_data="rate_set|0"),
        InlineKeyboardButton("Slower (-50%)", callback_data="rate_set|-50"), # Min rate is -100, but 0.5x is -50
        InlineKeyboardButton("Slowest (-100%)", callback_data="rate_set|-100")
    )
    return markup
...
def cmd_voice_rate(message):
    ...
    bot.send_message(
        message.chat.id,
        "🎙️ Changing voice speed. Select a preset below, or enter a number from -100 to +100 (0 is normal):",
...
def on_rate_set_callback(call):
    ...
    try:
        ...
        bot.answer_callback_query(call.id, f"✔️ Rate set to {rate_value}%")
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text=f"🔊 Voice speed set to *{rate_value}%*. Now you can send text for speech or use /Langtts to pick a voice.",
            parse_mode="Markdown"
        )
    except ValueError:
...
def cmd_voice_pitch(message):
    ...
    bot.send_message(
        message.chat.id,
        "🎙️ Changing voice pitch. Select a preset below, or enter a number from -100 to +100 (0 is normal):",
...
def on_pitch_set_callback(call):
    ...
    try:
        ...
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text=f"🔊 Voice pitch set to *{pitch_value}%*. Now you can send text for speech or use /Langtts to pick a voice.",
            parse_mode="Markdown"
        )
    except ValueError:
...
@bot.message_handler(commands=['Langtts'], regexp='^/Langtts$') # RENAMED
@bot.message_handler(func=lambda m: m.text == "🔊 Text-to-Speech") # New button text
def cmd_text_to_speech(message):
    ...
    # On /Langtts, set TTS mode but no voice yet, deactivate pitch/rate input mode
    user_tts_mode[user_id] = None
    user_pitch_input_mode[user_id] = None
    user_rate_input_mode[user_id] = None # NEW: Reset rate input mode

    bot.send_message(message.chat.id, "🗣️ Select the language of the text you want to convert to audio using the menu below:", reply_markup=make_tts_language_keyboard())
...
def on_tts_language_select(call):
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text=f"🎙️ Choose the voice that will read the language *{lang_name}* you selected using the buttons below:",
...
def on_tts_voice_change(call):
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text=f"🔊 Now using: *{voice}*. Current pitch: *{current_pitch}%*. Current speed: *{current_rate}%*. You can start sending text messages to convert them to speech.",
        parse_mode="Markdown"
    )
...
def on_tts_back_to_languages(call):
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text="🗣️ Select the language of the text you want to convert to audio using the menu below:",
...
async def synth_and_send_tts(chat_id: int, user_id: str, text: str):
    ...
    try:
        ...
        with open(filename, "rb") as f:
            bot.send_audio(chat_id, f, caption=f"🎤 Voice: {voice}, Pitch: {pitch}%, Speed: {rate}%") # NEW: Show rate in caption
...
@bot.message_handler(commands=['bottomtrnlang'], regexp='^/bottomtrnlang$') # RENAMED
@bot.message_handler(func=lambda m: m.text == "📝 Translate Language") # New button text
...
@bot.message_handler(commands=['bottomsumlang'], regexp='^/bottomsumlang$') # RENAMED
@bot.message_handler(func=lambda m: m.text == "📚 Summary Language") # New button text
...
@bot.message_handler(commands=['Langstt'], regexp='^/Langstt$') # RENAMED
@bot.message_handler(func=lambda m: m.text == "🗣️ Media Language") # New button text
def select_media_language_command(message):
    ...
    # Ensure TTS modes are OFF on /Langstt
    user_tts_mode[uid] = None
    user_pitch_input_mode[uid] = None
    user_rate_input_mode[uid] = None # NEW: Reset rate input mode
...
def button_summarize_handler(call):
    ...
    if preferred_lang:
        ...
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text="📄 Please select the language you want the summary in (you can also use /bottomsumlang for more options, or press 'Auto'):",
            reply_markup=markup,
            parse_mode="Markdown"
        )
    bot.answer_callback_query(call.id)
...
def callback_summarize_in(call):
    ...
    if lang_display_name == "Auto ⚙️":
        ...
        if transcription_lang:
            ...
            bot.answer_callback_query(call.id, "Cannot determine original transcription language. Please select a specific language using /bottomsumlang.")
            # If "Auto" fails and no media_language is set, inform the user to use /bottomsumlang
            bot.edit_message_text(
                chat_id=call.message.chat.id,
                message_id=call.message.message_id,
                text="⚠️ Cannot determine original transcription language. Please use /bottomsumlang to select a language, then try again."
            )
            return
    else:
        # This branch will primarily be hit if the user selected a language via /bottomsumlang and then interacted with a message.
...
def handle_summarize(message):
    ...
    if transcription_message_id:
        ...
        # User will be prompted to use /bottomsumlang for full list.
        markup = generate_language_keyboard("summarize_in", message.message_id) # Pass current message_id for the "Auto" button
        bot.send_message(
            message.chat.id,
            "📄 Please select 'Auto' for summary, or use /bottomsumlang to set your preferred summary language for future use:",
            reply_markup=markup,
            parse_mode="Markdown"
        )
...
def handle_text_for_tts_or_fallback(message):
    ...
    if user_rate_input_mode.get(uid) == "awaiting_rate_input":
        try:
            ...
            if -100 <= rate_val <= 100:
                ...
                bot.send_message(message.chat.id, "❌ Invalid speed. Please enter a number from -100 to +100 (0 is normal).")
    ...
    if user_pitch_input_mode.get(uid) == "awaiting_pitch_input":
        try:
            ...
            if -100 <= pitch_val <= 100:
                ...
                bot.send_message(message.chat.id, "❌ Invalid pitch. Please enter a number from -100 to +100 (0 is normal).")
    ...
    if user_tts_mode.get(uid):
        ...
        if saved_voice != "en-US-AriaNeural": # Default voice implies no explicit selection
            ...
            bot.send_message(
                ...
                "To convert text to speech, use the /Langtts command (or the '🔊 Text-to-Speech' button) first."
            )
...
def fallback_non_text_or_media(message):
    ...
    if message.document and not (message.document.mime_type.startswith("audio/") or message.document.mime_type.startswith("video/")):
        bot.send_message(
            message.chat.id,
            "❌ The document type you sent is not supported for transcription. Please send only voice messages, audio files, video notes, or video files. "
            "To convert text to speech, use the /Langtts command."
        )
        return
    ...
    bot.send_message(
        message.chat.id,
        "🤷‍♀️ I only process voice messages, audio files, or videos for transcription. "
        "For text-to-speech, please use the /Langtts command first. Please send a supported type."
    )
...
def set_bot_commands():
    ...
    commands = [
        ...
        BotCommand("bottomtrnlang", "Set preferred translation language"), # RENAMED
        BotCommand("bottomsumlang", "Set preferred summary language"), # RENAMED
        BotCommand("Langstt", "Set media transcription language"), # RENAMED
        BotCommand("Langtts", "Set Text-to-Speech language & voice"), # RENAMED
        BotCommand("voice_pitch", "Adjust TTS voice pitch"),
        BotCommand("voice_rate", "Adjust TTS voice speed"), # NEW command
        BotCommand("userinfo", "View your bot usage and settings"), # NEW command
        BotCommand("help", "How to use the bot"),
        BotCommand("privacy", "Read privacy notice"),
        # These are now primarily accessed via inline buttons after transcription
        # but kept as commands for direct access/reply
