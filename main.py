...
def set_bot_info():
    ...
    # bot.set_my_commands(commands) # kudar

    # bot.set_my_short_description( # kudar
    #     "I can transcribe audio/video to text and download videos from links."
    # )

    # bot.set_my_description( # kudar
...
def handle_social_video_url(msg):
    ...
    try:
        ...
        # Use yt-dlp to download the video and extract metadata
        unique_id = str(uuid.uuid4())
        temp_output_path = os.path.join(DOWNLOAD_DIR, f"{unique_id}.%(ext)s")
        
        ydl_opts = {
            'format': 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best', # Prioritize mp4 for broad compatibility
            ...
            'writeinfojson': True, # Write video info to a .info.json file
            'writethumbnail': True, # Write thumbnail to a .webp, .jpeg or .png file
            'force_generic_extractor': False, # Allow platform-specific extractors
            'extractor_args': {
                'instagram': {
                    'username': '', # You might need to provide credentials for private videos, but generally not needed for public ones.
                    'password': '',
                },
                'facebook': {
                    'username': '', # Same for Facebook
                    'password': '',
                }
            }
        }
        ...
        info_json_path = None
        downloaded_file_path_base = None

        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info_dict = ydl.extract_info(url, download=True)
            downloaded_file_path_base = ydl.prepare_filename(info_dict)
            video_path = downloaded_file_path_base # This will be the actual downloaded video file path

            # Adjust the downloaded_file_path_base to get the correct path if .mp4 was appended
            if not video_path.endswith('.mp4'):
                 video_path = f"{video_path}.mp4" # Assume it merged to .mp4

            info_json_path = f"{downloaded_file_path_base}.info.json"
            description_path = f"{downloaded_file_path_base}.description"
            thumbnail_path = None
            
            # Find the actual thumbnail file
            for ext in ['webp', 'jpeg', 'png', 'jpg']: # Common thumbnail extensions
                possible_thumbnail_path = f"{downloaded_file_path_base}.{ext}"
                if os.path.exists(possible_thumbnail_path):
                    thumbnail_path = possible_thumbnail_path
                    break

        caption_text = POWERED_BY_CAPTION
        
        # Try to read info from .info.json for caption and hashtags
        if os.path.exists(info_json_path):
            try:
                ...
                tags = video_info.get('tags', []) # For YouTube mostly, might not be common for others

                if title:
                    caption_text = f"**{title}**\n\n"

                if description:
                    # Limit description length if too long for caption
                    if len(description) > 500: # Arbitrary limit for brevity
                        caption_text += f"{description[:497]}...\n\n"
                    else:
                        caption_text += f"{description}\n\n"
                
                hashtags_from_description = re.findall(r'#[a-zA-Z0-9_]+', description or '')
                all_hashtags = list(set(tags + hashtags_from_description)) # Combine and unique
                if all_hashtags:
                    caption_text += " ".join(all_hashtags[:10]) + "\n\n" # Limit to first 10 hashtags

                caption_text += POWERED_BY_CAPTION

            except Exception as e:
                logging.warning(f"Error reading video info JSON: {e}")
            ...
            try:
                with open(description_path, 'r', encoding='utf-8') as f:
                    ...
                    if description:
                        ...
                        hashtags_from_description = re.findall(r'#[a-zA-Z0-9_]+', description)
                        if hashtags_from_description:
                            caption_text += " ".join(hashtags_from_description[:10]) + "\n\n"
                        caption_text += POWERED_BY_CAPTION
            except Exception as e:
                logging.warning(f"Error reading video description file: {e}")
        ...
        # Send with thumbnail if available, otherwise just video
            ...
            try:
                with open(video_path, 'rb') as video_file, open(thumbnail_path, 'rb') as thumb_file:
                    bot.send_video(
                        ...
                        reply_markup=get_more_info_button(),
                        parse_mode="Markdown",
                        thumbnail=thumb_file.read() # Send thumbnail as bytes
                    )
            except Exception as e:
                logging.error(f"Error sending video with thumbnail: {e}. Trying without thumbnail.")
        ...
        error_message = str(e)
        if "instagram" in url.lower() and ("private" in error_message or "login" in error_message):
            bot.send_message(msg.chat.id, "❌ I cannot download private Instagram videos. Please ensure the video is public.")
        elif "facebook" in url.lower() and ("private" in error_message or "login" in error_message):
            bot.send_message(msg.chat.id, "❌ I cannot download private Facebook videos. Please ensure the video is public.")
        else:
            bot.send_message(msg.chat.id, f"❌ An error occurred during video download: {e}. Please check the link or try again later.")
    except Exception as e:
        logging.error(f"General error processing social media video link {url}: {e}")
        ...
        if downloaded_file_path_base:
            for ext in ['mp4', 'webm', 'mkv', 'avi', 'mov', 'flv', 'ogg', 'description', 'info.json', 'webp', 'jpeg', 'png', 'jpg']:
                file_to_remove = f"{downloaded_file_path_base}.{ext}"
                if os.path.exists(file_to_remove):
                    os.remove(file_to_remove)
        # Also try to remove the specific video_path if it's different and exists
