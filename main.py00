...
# ======= API Tokens & Config =======
BOT_TOKEN = "7790991731:AAFgEjc6fO-iTSSkpt3lEJBH86gQY5nIgAw"
...
def send_language_prompt(message):
    ...
    if chat_id == ADMIN_ID:
        ...
        bot.send_message(
            chat_id,
            "👋 Welcome! Choose your Media (Voice, Audio, Video) file language for transcription using the buttons below:",
...
def handle_media_and_text(message):
    ...
    try:
        ...
        if file_info.file_size > 20 * 1024 * 1024:
            bot.delete_message(chat_id, processing_msg.message_id)
            bot.reply_to(message, "⚠️ File is too large. Max allowed size is 20MB.") # Reply to the original message
            return
